...
describe("Integration Test Suite", () => {
  it("Testing full functionality of the code", (done) => {
    ...
    const test_bucket = AWS_Settings.BUCKET_NAME_TEST;
    ...
    listS3BucketFiles(s3, test_bucket)
      .then((keys) => {
        ...
        return openConcurrentDownloads(
          BPromise,
          keys,
          NUMBER_OF_CONCURRENCY_S3_DOWNLOAD,
          (key) => downloadFileFromS3(s3, test_bucket, key)
        );
      })
      .then((results) => {
      ...
      .then((fileSettledResult) => {
        const filenamesStr = fileSettledResult
          .map(({ value }) => value)
          .toString();
