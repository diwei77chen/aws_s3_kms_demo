 ...
 */
const listS3BucketFiles = (s3, bucketName) =>
  s3
 ...
 */
const downloadFileFromS3 = (s3, bucketName, key) =>
  s3
 ...
 */
const encryptData = (kms, cmk, data) =>
  kms
    .encrypt({ KeyId: cmk, Plaintext: data })
    .promise()
  ...
  return listS3BucketFiles(s3, bucketName)
    .then((keys) =>
      openConcurrentDownloads(
        BPromise,
        keys,
        NUMBER_OF_CONCURRENCY_S3_DOWNLOAD,
        (key) => downloadFileFromS3(s3, bucketName, key)
      )
    )
    .then((results = []) =>
      Promise.allSettled(
        results.map(({ key, Body }) => writeToFile(fs, pathToSave, key, Body))
      )
    )
    .then((fileSettledResult) =>
      fileSettledResult.map(({ value }) => value).toString()
    )
    .then((filenames) => encryptData(kms, CMK, filenames))
    .then((encryptedData = "") =>
      writeToFile(fs, pathToSave, outputFilename, encryptedData)
    );
};
